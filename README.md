...
**Export FIT pair** is available in Overview and Calibration after recording stops. It downloads a ZIP containing `trainer.fit`, `reference.fit`, and export notes. Both feeds use one-second sample averages on a shared UTC clock, preserving gaps and zeros and including available cadence and left/right balance. Export uses the full selected recording without calibration, time offsets, or trimming. Keep the original CSV for full-resolution samples.

**Exclude zero-power readings** is now optional in Calibration, FIT export, and the Overview timeline. Calibration defaults to excluding any fitting window containing a zero from either feed; toggling it immediately updates the regression. FIT export defaults to keeping zeros, with an option to omit zero-containing seconds independently for each feed while preserving timestamps and activity duration. Overview filtering is graph-only: ride averages and power curves still count coasting. Missing values always remain excluded or appear as gaps. Original recordings stay unchanged.
...
All trainer feeds now use the same one-second arithmetic sample mean. At timestamp 10, the app averages every received trainer reading after 9 and through 10, whether there are ten, three, or just one. No rate detection, interpolation, future readings, or filling empty seconds is used. An initial reading can display at time zero without adding a fictitious second of coverage. Left/right power shares the total’s samples and smoothing, so the traces stay aligned. Calibration, delay estimation, average-power comparison, and power curves use the same baseline. Small offset changes can select the same packets; the delay estimator prefers the smallest shift when scores tie. Calibration uses one power-band breakdown table, and **Estimate delay** is available directly in the time-alignment controls.
...
Built from private source revision `3f3bda3` with TinyGo 0.42.0 (Go 1.26.0) and Binaryen 132. The app loads its TinyGo worker from `wasm-tinygo/`. Legacy standard-Go Wasm assets have been removed. Published assets only; not an open-source release.
