...
Built from private source revision `54e9b11` with TinyGo 0.42.0 (Go 1.26.0) and Binaryen 132. The app loads the smaller TinyGo worker from `wasm-tinygo/`; the previous `wasm/` assets remain available for cached older pages. Published assets only; not an open-source release.
